...
- **Flexible dates** — pick an *earliest month* and a *latest month*, plus a
  trip-length band (e.g. 10–18 nights).
- **Price-calendar engine** 📅 — by default the app uses Skyscanner's
  price-calendar: **one cheap call per month** returns the cheapest fare for
  *every* day, so it finds the genuinely cheapest departure days (matching the
  numbers you see on skyscanner.ie) and then spends a full search only on those
  few days. Far less quota than blindly sampling dates. Turn it off in Settings
  to use the legacy even-sampling scan.
...
- In **price-calendar mode** (default), a scan costs roughly *one call per
  month* in the window plus a full search (~2–5 calls) for each of the cheapest
  days you ask it to detail-check — set that with **"Detail-check N cheapest
  days"**. This is the most quota-friendly way to run it.
- In **legacy sampling mode**, each scan is capped by **"Max searches per scan"**
  and samples dates evenly to fit that budget, so one scan can't drain your whole
  month. Lower it (e.g. 5) to stretch the free tier further.
