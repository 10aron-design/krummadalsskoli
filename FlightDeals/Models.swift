...
struct SearchConfig: Codable, Equatable {
    ...
    // MARK: Price-calendar engine

    /// Use Skyscanner's price-calendar endpoint instead of blind date sampling.
    /// ONE cheap call per route×month returns the cheapest fare for *every* day,
    /// so we spot the genuinely cheapest days (matching Skyscanner's numbers) and
    /// only spend expensive detail searches on those. Turn OFF to fall back to the
    /// old even-sampling scan.
    var usePriceCalendar: Bool = true
    /// When the calendar engine runs, how many of the cheapest days to fully
    /// detail-check (each detail check costs ~2–5 RapidAPI calls).
    var calendarCandidateDays: Int = 4
...
// MARK: - One day on Skyscanner's price calendar.
// The getPriceCalendar endpoint returns the cheapest fare for every day in a
// month with a single call, letting us find the cheapest day without searching
// each date individually.

struct CalendarDay: Equatable {
    let date: Date
    let price: Double
    let group: String   // Skyscanner's banding: "low" / "medium" / "high"
}
