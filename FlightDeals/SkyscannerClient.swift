...
protocol FlightProvider {
    ...
    /// Cheapest fare per day across the month starting at `month`, in ONE call.
    /// Used to find the genuinely cheapest departure days before spending
    /// expensive detail searches on them.
    func priceCalendar(origin: String,
                       destination: String,
                       month: Date,
                       currency: String) async throws -> [CalendarDay]
}
...
actor SkyscannerClient: FlightProvider {
    ...
    // MARK: Price calendar (cheapest fare per day, one call per month)
    ...
    func priceCalendar(origin: String,
        ...
        let req = try makeRequest(
            ...
            query: [
                .init(name: "originSkyId", value: from.skyId),
                .init(name: "destinationSkyId", value: to.skyId),
                .init(name: "fromDate", value: month.ymd),
                .init(name: "currency", value: currency),
                .init(name: "market", value: creds.market),
                .init(name: "countryCode", value: creds.market)
            ])
        ...
        return Self.parseCalendar(data: data)
    }

    /// getPriceCalendar returns { data: { flights: { days: [ {day, group, price} ] } } }.
    /// `price` may be a raw number or wrapped — parse defensively, skip 0/no-price.
    static func parseCalendar(data: Data) -> [CalendarDay] {
        guard
            let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
            let d = obj["data"] as? [String: Any],
            let flights = d["flights"] as? [String: Any],
            let days = flights["days"] as? [[String: Any]]
        else { return [] }
        ...
        return days.compactMap { day -> CalendarDay? in
            guard let ds = str(day["day"]), let date = f.date(from: ds) else { return nil }
            // price can be a bare number or { "raw": 123.4, "price": "€123" }.
            let price: Double?
            if let p = dbl(day["price"]) { price = p }
            else if let po = day["price"] as? [String: Any] { price = dbl(po["raw"]) }
            else { price = nil }
            guard let amount = price, amount > 0 else { return nil }
            return CalendarDay(date: date, price: amount, group: str(day["group"]) ?? "")
        }
    }
